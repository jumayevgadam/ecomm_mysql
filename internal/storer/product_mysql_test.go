	...
	tcs := []struct {
	...
	}{
		{
			...
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
				...
				if err != nil {
					t.Fatalf("error creating product: %v", err.Error())
				}
...
func TestGetProduct(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			name: "success",
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
				rows := sqlmock.NewRows([]string{"id", "name", "image", "category", "description", "rating", "num_reviews", "price", "count_in_stock"}).
					AddRow(1, p.Name, p.Image, p.Category, p.Description, p.Rating, p.NumReviews, p.Price, p.CountInStock)
